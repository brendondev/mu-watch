// watcher v1.3.1 — CF PAT detected → add proxy support, mobile UA toggle, headless toggle, full debug
...
const MOBILE_UA = process.env.MOBILE_UA === "1"; // 1 = finge iPhone
...
const PROXY_SERVER = process.env.PROXY_SERVER || "";          // ex: http://host:port
const PROXY_USERNAME = process.env.PROXY_USERNAME || "";
const PROXY_PASSWORD = process.env.PROXY_PASSWORD || "";

console.log("watcher version v1.3.1 (CF PAT, proxy-ready, debug)");
  ...
  for (const f of page.frames()) {
    try {
      if (/usercentrics|consent|cookiebot/i.test(f.url())) {
        ...
        await btn.click({ timeout: 800 }); return;
      }
    } catch {}
  }
}
  ...
  while (Date.now() - start < timeoutMs) {
    const ok = await page.evaluate((n) => {
      ...
      return blocks.some((c) => {
        const b = c.querySelector("b"); const txt = (b?.textContent || c.textContent || "").trim();
...
function attachDebug(page, tag = "") {
  ...
  page.on("requestfailed", (req) => console.log(
    `[REQFAIL${tag}] ${req.method()} ${req.url()} — ${req.failure()?.errorText || "unknown"}`
  ));
  page.on("response", async (res) => {
    try {
      const url = res.url(); const st = res.status();
      if (st >= 400) console.log(`[HTTP${st}${tag}] ${url}`);
      const ct = (res.headers()["content-type"] || "").toLowerCase();
      if (ct.includes("application/json")) {
        const t = await res.text(); console.log(`[JSON${tag}] ${url} bytes=${t.length}`);
      }
    } catch {}
  });
}
...
function extractFromJson(any, nick) {
  const res = {}; const visited = new Set(); const targetNick = (nick || "").toLowerCase();
  function walk(obj) {
    if (!obj || typeof obj !== "object" || visited.has(obj)) return; visited.add(obj);
    ...
    for (const [k, v] of Object.entries(obj)) {
      ...
      if (typeof v === "string") {
        const vs = v.trim();
        if (!res.status && /^(online|offline)$/i.test(vs) && /status|online|offline/.test(key)) res.status = /online/i.test(vs) ? "Online" : "Offline";
        if (!res.location && /(location|localiza)/.test(key) && vs.length > 0) res.location = vs;
      }
      if (typeof v === "boolean" && !res.status && /(status|online)/.test(key)) res.status = v ? "Online" : "Offline";
      if (typeof v === "object" && v) walk(v);
    }
    if (matchNick) {
      if (!res.status) {
        const s = obj.status ?? obj.state ?? obj.online;
        if (typeof s === "string") res.status = /online/i.test(s) ? "Online" : (/offline/i.test(s) ? "Offline" : undefined);
        else if (typeof s === "boolean") res.status = s ? "Online" : "Offline";
      }
      if (!res.location) {
  ...
  try { walk(any); } catch {}
  if (res.location) { if (/^Lorencia$/i.test(res.location)) res.location = "Hidden 🔐"; if (/^Noria$/i.test(res.location)) res.location = "Noria 🌸"; }
  return (res.status || res.location) ? res : null;
}
  ...
  while (Date.now() < deadline) {
    try {
      const resp = await page.waitForResponse(async (r) => {
        try {
          const url = new URL(r.url()); const h = url.host;
      ...
      try {
        const data = JSON.parse(txt); const got = extractFromJson(data, nick);
        if (got && (got.status || got.location)) return got;
      } catch {
        const maybe = txt.split(/\n(?=\{|\[)/).map((s) => s.trim()).filter(Boolean);
        for (const chunk of maybe) {
          try { const data2 = JSON.parse(chunk); const got2 = extractFromJson(data2, nick); if (got2 && (got2.status || got2.location)) return got2; } catch {}
        }
      }
    } catch {}
  }
...
async function extractOnce(page, nick) {
  return await page.evaluate((n) => {
    ...
    const header = nameBlocks.find((c) => {
      const b = c.querySelector("b"); const txt = norm(b?.textContent || c.textContent || "");
      return new RegExp(`\\b${n}\\b`, "i").test(txt);
    }) || null;
    ...
    if (header) {
      ...
      if (status === "—") {
        const img = header.querySelector('img[alt]'); const alt = img?.getAttribute("alt") || "";
        if (/^online$/i.test(alt)) status = "Online"; else if (/^offline$/i.test(alt)) status = "Offline";
      }
    }
    ...
    const pickLocation = (root) => {
      ...
      for (let i = 0; i < spans.length; i++) {
        ...
        if (/^Localiza(?:ç|c)ão\s*:?$|^Location\s*:?$|^Localização$/i.test(t)) {
          ...
          const v = norm(next?.textContent || ""); if (v) return v;
        }
      }
    ...
    let location = "—";
    const infoRoot = document.querySelector(".CharPage_char-info__EW_Lb") || document.querySelector('[class*="CharPage_char-info__"]') || document;
  ...
  for (let i = 1; i <= maxTries; i++) {
    try {
      ...
      await page.evaluate(() => {
        ...
        const target = items.find((el) => /GUILDWAR/.test(norm(el.textContent))) || items.find((el) => /X\s*-\s*50/.test(norm(el.textContent)));
        if (target) { target.scrollIntoView({ block: "center" }); target.dispatchEvent(new MouseEvent("click", { bubbles: true })); }
      });
...
(async () => {
  ...
  await context.route("**/*", (route) => {
    const req = route.request(); const type = req.resourceType(); const url = req.url();
    try {
      const u = new URL(url); const h = u.host;
      if (/(googletagmanager|google-analytics|gtag|facebook|hotjar|yandex|tiktok)/i.test(h)) { if (DEBUG) console.log(`[ROUTE-ABORT] type=${type} url=${url}`); return route.abort(); }
      if (/^challenges\.cloudflare\.com$/i.test(h)) { return route.continue(); }
      if (allowHost(h)) { return route.continue(); }
      if (["script", "stylesheet", "image", "font", "media"].includes(type)) { if (DEBUG) console.log(`[ROUTE-ABORT] type=${type} url=${url}`); return route.abort(); }
    } catch {}
    return route.continue();
  });

  // Warm
  const warm = await context.newPage(); attachDebug(warm, ":warm");
  warm.setDefaultTimeout(6000); warm.setDefaultNavigationTimeout(16000);
  ...
  for (let i = 0; i < CONCURRENCY; i++) {
    const p = await context.newPage(); attachDebug(p, `:w${i}`);
    p.setDefaultTimeout(6000); p.setDefaultNavigationTimeout(16000);
    pages.push(p);
  }

  const changes = []; let idx = 0;

  async function processNick(page, nick) {
    const url = `${BASE}${PATH}${encodeURIComponent(nick)}`;
    await page.goto(url, { waitUntil: "domcontentloaded" });
    const challenged1 = await waitCloudflare(page, 12000); if (DEBUG && challenged1) console.log(`[CF] challenge detectado em ${url}`);

    if (await isX5(page)) {
      await ensureX50Once(page).catch(() => {});
      await page.goto(url, { waitUntil: "domcontentloaded" });
      const challenged2 = await waitCloudflare(page, 10000); if (DEBUG && challenged2) console.log(`[CF] challenge detectado após reload ${url}`);
    }
    ...
    let dataFromJSON = (jsonResult.status === "fulfilled") ? jsonResult.value : null;
    ...
    for (const d of delays) { if (d) await sleep(d); data = await extractOnce(page, nick); if (data.ok && data.location !== "—") break; }
    ...
    if (!data.ok) {
      if (DEBUG) {
        try {
          const ts = Date.now(); const base = `debug_${encodeURIComponent(nick)}_${ts}`;
          await page.screenshot({ path: `${base}.png`, fullPage: true });
          const html = await page.content(); await fs.writeFile(`${base}.html`, html, "utf8");
  ...
  async function worker(p) {
    while (idx < list.length && Date.now() < HARD_DEADLINE) {
      const myIdx = idx++; const nick = list[myIdx];
      try {
        const cur = await processNick(p, nick);
        if (!cur.ok) console.log(`skip ${nick}: ${cur.error || "no-data"}`);
        else {
      ...
      } catch (e) { console.log(`fail ${nick}: ${e?.message || e}`); }
      await sleep(80 + Math.random() * 120);
    }
    if (Date.now() >= HARD_DEADLINE) console.log("time budget reached — exiting gracefully");
  }
  ...
  if (changes.length) {
    ...
    const embeds = changes.map((c) => {
      ...
      return { title: `${titleEmoji} ${c.nick}`, url, color,
        description: `${statusLine}\n${locLine}\n\n⏱️ <t:${unix}:f> • <t:${unix}:R>`,
        footer: { text: `watcher v1.3.1 • GuildWar (X-50) • CONCURRENCY=${CONCURRENCY}` },
    ...
    for (let i = 0; i < embeds.length; i += 10) {
      await postDiscord({ username: "MU Watcher X-50", content: i === 0 ? header : undefined, embeds: embeds.slice(i, i + 10) });
      await sleep(300);
    }
  } else { console.log("no changes to report"); }

  await saveState(state);
  if (DEBUG) { try { await context.tracing.stop({ path: "trace.zip" }); } catch {} }
  await browser.close();
})();
